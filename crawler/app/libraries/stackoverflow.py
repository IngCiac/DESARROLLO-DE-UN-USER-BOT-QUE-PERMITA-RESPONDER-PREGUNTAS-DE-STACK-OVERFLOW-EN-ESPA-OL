...
class StackOverflow:
    ...
    def get_questions(self, page = 1, file = 'questions.txt'):
        ...
        for index, question_summary in enumerate(divs):
            logger.debug('question {page} {index}/{divs_len}'.format(
                page = page,
                index = index + (page - 1) * divs_len,
                divs_len = page * divs_len
            ))
            ...
            if session.query(Question)\
                ...
                content = responses_page.find('div', class_ = 'post-layout').text
